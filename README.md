...
- 显示额外充值余额和预计扣费
- 显示当前 token 消耗速度、今日安全速度和预算可持续时间
- 显示 7 日成本趋势和 token 趋势
- 显示不同工具的用量占比
- 支持浏览器提示音测试
- 支持任务完成、审批确认、额度告警等提醒规则展示
- 区分不同工具的数据源接入方式
- 支持在本机测试 OpenAI-compatible API 供应商连接
...
- 当前运行任务的 token/min
- 今日安全 token/min
- 按当前速度连续 1 小时的预计 token
- 预算可持续时间

当前版本的速度来自任务队列里的演示任务，后续接入真实日志后可以改为实时计算。
...
每个供应商都可以编辑 Base URL 和模型名，然后点击 `测试连接`。API Key 只在测试时输入，不会写入仓库。
